              ...
              <div className="space-y-2">
                {AVAILABLE_MODELS.map((m) => (
                  <div 
                    ...
                    className={`p-3 rounded-lg border cursor-pointer transition-all relative overflow-hidden group/model ${
                      config.model === m.id 
                  ...
                  >
                    <div className="flex items-center justify-between mb-1 relative z-10">
                      <div className="flex items-center gap-2 overflow-hidden flex-1">
                        <span className={`font-medium text-sm truncate ${config.model === m.id ? 'text-indigo-300' : 'text-zinc-300'}`}>
                          {m.name}
                        </span>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0 ml-auto mr-1">
                          {m.supportsThinking && (
                            <div 
                              className="p-1 rounded-md bg-purple-500/15 text-purple-400 ring-1 ring-purple-500/20 group-hover/model:ring-purple-500/40 transition-all shadow-[0_0_8px_rgba(168,85,247,0.1)]" 
                          ...
                          {m.supportsSearch && (
                            <div 
                              className="p-1 rounded-md bg-blue-500/15 text-blue-400 ring-1 ring-blue-500/20 group-hover/model:ring-blue-500/40 transition-all shadow-[0_0_8px_rgba(59,130,246,0.1)]" 
                              title="Supports Google Search"
                            >
                              <Globe size={11} />
                            </div>
                          )}
                      ...
                      {config.model === m.id && (
                        <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 shadow-[0_0_8px_rgba(129,140,248,0.6)] shrink-0 ml-1" />
                      )}
                    </div>
                    <p className="text-[11px] text-zinc-500 relative z-10 line-clamp-1 group-hover/model:text-zinc-400 transition-colors">
                      {m.description}
                    </p>
                  </div>
                ))}
